...
public class IdentifierRules {
    ...
    public List<String> parseQualifiedIdentifier(String fqn) {
        final String orig = fqn;
        final String delim = identifierDelimiter();
        final String lead = leadingQuoteString();
        final String trail = trailingQuoteString();
        final List<String> parts = new ArrayList<>();
        int index;
        String segment;
        do {
            if (!lead.equals(UNSUPPORTED_QUOTE) && fqn.startsWith(lead)) {
                final int end = fqn.indexOf(trail, lead.length());
                if (end < 0) {
