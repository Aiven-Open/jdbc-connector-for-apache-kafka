 ...
 * {@code META-INF/services/io.aiven.connect.jdbc.dialect.DatabaseDialectProvider}.
 *
 * <p>For example, all of the dialects providers provided by the Aiven JDBC connector are listed
 * in the {@code META-INF/services/io.aiven.connect.jdbc.dialect.DatabaseDialectProvider}.
...
public abstract class DatabaseDialectProvider {
    ...
    protected DatabaseDialectProvider(final String name) {
        assert name != null;
        this.name = name;
    }
    ...
    public abstract static class FixedScoreProvider extends DatabaseDialectProvider {
        ...
        @Override
        public int score(final JdbcUrlInfo jdbcInfo) {
            return score;
        }
    }
}
